...
import ResultFinalOwn from '@/components/ResultFinalOwn.vue'
...
<template>
    ...
    <Panel v-model="panelVisible" @close="closePanel">
        ...
        <template #footer>
            <Button v-for="(menu, index) in menus" :key="menu.key" @click="clickMenu(index)" class="menu-btn" :disabled="menu.key === activeMenu">
                {{ menu.title }}
            </Button>
        </template>
    </Panel>
